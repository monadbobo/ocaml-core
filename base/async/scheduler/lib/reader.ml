...
let convert file f annot_sexp =
  ...
  with Of_sexp_error _ as exn ->
    Error (Load_sexp (file, `Error exn))
;;
...
let load_sexp_exn ?exclusive file f =
  load_sexp ?exclusive file f >>| Result.ok_exn
;;
...
let load_sexps_exn ?exclusive file f =
  load_sexps ?exclusive file f >>| Result.ok_exn
;;
