...
module Rpc_result : sig
  ...
  let make_try_with try_with (>>|) constructor f =
    try_with f >>| function
      | Ok x -> x
      | Error e -> Error (constructor (Exn.sexp_of_t e))

  let try_with f = make_try_with
  ...
  let try_with_bin_io f = make_try_with
    Result.try_with
    (fun x f -> f x)
...
module Server : sig
  ...
  let apply_implementation
    ...
    | Implementation.F.Rpc f ->
      let data =
        Rpc_result.try_with (fun () ->
          defer_result (f connection_state query.Query.data))
      in
      data >>> write_response
    | Implementation.F.Pipe_rpc f ->
      match of_bigstring Stream_query.bin_t query.Query.data with
      | Error _err -> ()
      | Ok `Abort ->
        Option.iter (Hashtbl.find open_streaming_responses
                       (query.Query.tag, query.Query.id))
          ~f:(fun i -> Ivar.fill_if_empty i ());
      | Ok (`Query data) ->
        begin
          let user_aborted = Ivar.create () in
          Hashtbl.replace open_streaming_responses
            ~key:(query.Query.tag, query.Query.id) ~data:user_aborted;
          ...
          let data = Rpc_result.try_with (fun () -> defer_result (
            f connection_state query.Query.id data ~aborted
          ))
          in
          data >>> fun data ->
          begin
            let remove_streaming_response () =
              Hashtbl.remove open_streaming_responses (query.Query.tag, query.Query.id)
            in
            match data with
            | Error err ->
  ...
  type t =
    {
      ...
      open_streaming_responses : (Rpc_tag.t * Query_id.t, unit Ivar.t) Hashtbl.t;
      handle_query             :
           (query:Query.t
            -> writer:Writer.t
            -> write_response:(Writer.t -> Response.t -> unit)
  ...
  let create ?server ~connection_state ?max_message_size:max_len reader writer =
    ...
    let result =
      ...
      | `Ok header ->
        ...
        in
        let stop_heartbeats = Ivar.create () in
        ...
        in
        Stream.iter (Monitor.errors (Writer.monitor t.writer)) ~f:(fun error ->
          close t;
          Error.raise (Error.Write_error (Exn.sexp_of_t error))
        );
        begin
          let monitor = Monitor.create ~name:"RPC connection loop" () in
          Stream.iter (Monitor.errors monitor) ~f:(fun exn ->
            Ivar.fill_if_empty stop_heartbeats ();
            let error = match exn with
              | Error.Rpc error -> error
              | exn -> Error.Uncaught_exn (Exn.sexp_of_t exn)
            in
            (* clean up open streaming responses *)
            Hashtbl.iter t.open_queries ~f:(fun ~key:query_id ~data:response_handler ->
              Deferred.ignore
                (response_handler
                   { Response.
                     id = query_id;
          ...
          Scheduler.within ~monitor (fun () ->
            Clock.every ~stop:(Ivar.read stop_heartbeats) (Span.of_sec 10.0) heartbeat;
            loop (read_message ()))
        end;
        Ok t
...
module Rpc = struct
  ...
  let implement t f =
    let f c query_bigstring =
      of_bigstring t.bin_query query_bigstring >>|~ fun query ->
  ...
  let dispatch t conn query =
    ...
    let query =
      { Query.
        ...
        version = t.version;
        id = Query_id.create ();
        data = to_bigstring t.bin_query query;
      }
    in
    ...
    begin
      ...
      | Ok () -> ()
      | Error `Closed -> Ivar.fill ivar (Error Error.Connection_closed)
    end;
    Ivar.read ivar >>| Rpc_result.to_exn_result
...
module Streaming_rpc = struct
  ...
  let implement t f =
    let f c query_id query_bigstring ~aborted =
  ...
  let dispatch t conn query =
    ...
    let query =
      { Query.
        ...
        version = t.version;
        id = Query_id.create ();
        data = to_bigstring Stream_query.bin_t
    ...
    let response_handler response =
      let read_updates pipe_w response =
        ...
        | Ok data ->
          begin
            match of_bigstring Stream_response_data.bin_t data with
            | Error err ->
              Pipe.close pipe_w;
              return (`Die err)
            | Ok `Eof ->
            ...
            | Ok (`Ok data) ->
              begin
                match of_bigstring t.bin_update_response data with
                | Error err ->
                  Pipe.close pipe_w;
                  return (`Die err)
                | Ok data ->
        ...
        | Ok initial_msg -> begin
          ...
          | Ok initial_msg -> begin
            ...
            | Ok initial ->
              ...
              Ivar.fill ivar (Ok (Ok (
                initial_msg.Initial_message.id, initial, pipe_r)));
