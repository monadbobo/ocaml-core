...
module Caller_converts = struct
  ...
  module type S = sig
    ...
    type response
    val dispatch_multi :
      version:int -> Rpc.Connection.t -> query -> (response, exn) Result.t Deferred.t
...
module Callee_converts = struct
  ...
  module Make (Model : sig
    ...
    type implementer =
      { implement : 's. log_version:(int -> unit) -> 's impl -> 's Rpc.Implementation.t }
...
module Menu = struct
  ...
  let add impls =
    let menu =
      List.map impls ~f:(fun i ->
        match Rpc.Implementation.description i with
          {Rpc.Implementation.Description.name; version} -> (name, version))
    in
  ...
  let request conn =
    Rpc.Rpc.dispatch Current_version.rpc conn ()
    >>| fun result ->
