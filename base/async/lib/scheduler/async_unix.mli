(** Async_unix provides an interface to many of the functions in OCaml's standard Unix
...
(** Returns a deferred that becomes determined when the current process
    becomes a child of init(8).  This is useful to determine if one's
    parent has died, because in that case init will becomes one's
    parent.
...
val child_of_init : ?poll_delay:Time.Span.t -> unit -> unit Deferred.t
...
val openfile : string -> mode:open_flag list -> perm:file_perm -> Fd.t Deferred.t
...
(** [with_file_read file ~f] opens [file] for reading and applies [f] to the resulting
    file descriptor.  When the result of [f] becomes determined, [with_file] closes
    the file descriptor and returns the result of [f]. *)
val with_file_read :
  string
  -> f:(Fd.t -> 'a Deferred.t)
  -> 'a Deferred.t
...
val link :
  ?force : bool -> target:string -> link_name:string -> unit -> unit Deferred.t
...
val mkdir : string -> perm:file_perm -> unit Deferred.t

val mkdir_p : string -> perm:file_perm -> unit Deferred.t
...
(** [waitpid pid] is like [wait (`Pid pid)] but does not lock an Async Helper_thread.t. *)
...
module Socket : sig
  ...
  *)
  ...
  val shutdown :
    ('a, 'addr) t -> [ `Receive | `Send | `Both ] -> unit
...
(** [get_inet_addr hostname] does a DNS lookup of hostname and returns the resulting IP
    address.  The implemenation sequentializes all calls so that only a single call is
    active at a time.  The is because we've observed thread safety issues with certain
    versions of winbind using "wins" name resolution.
*)
val get_inet_addr : string -> Inet_addr.t Deferred.t
