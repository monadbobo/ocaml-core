...
(** [pipe ?max_batch ?allow_straight_through ()] returns [(p, put, close)], where [p] is a
    pipe reader that receives all of the values supplied to [put] and is closed when
    [close] is called.
    ...
    Calling [put] after [close] is an error.

    If [allow_straight_through = true], the default, then [put] attempts to acquire
    Async's lock, and if it can, then it extends the stream and runs a cycle.  If [not
    allow_straight_through] or the lock cannot be acquired, then [put] puts the element on
    an squeue.  [pipe ()] creates a helper thread that process this squeue and handles all
    work placed on it.

    [max_batch] lmits the number of stream elements that will be processed in a single
    Async cycle. *)
val pipe :
  ?max_batch:int
  -> ?allow_straight_through:bool
  -> unit
  -> 'a Pipe.Reader.t * ('a -> unit) * (unit -> unit)
