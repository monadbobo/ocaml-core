(** Threading model:
    ...
    Async world.  They do this by calling [In_thread.run_in_async*], which acquires the
...
val go :
  ?raise_unhandled_exn:unit
  -> unit
  -> never_returns

(** [go_main] is like [go], except that one supplies a [main] function that will be run
    to initialize the async computation, and that [go_new] will fail if any async
    has been used prior to [go_new] being called. *)
val go_main :
  ?raise_unhandled_exn:unit
  -> main:(unit -> unit)
  -> unit
...
(** [go_in_thread ()] starts the async scheduler in another thread. *)
(* val go_in_thread : unit -> unit *)
...
(** [cycle_start ()] returns the result of Time.now() called at the beginning
    of cycle.
*)
val cycle_start : unit -> Time.t


(** [cycle_times ()] returns a stream that will have one element for each cycle
    that Async runs, with the amount of time that the cycle took (as determined
    by calls to Time.now at the beginning and end of the cycle).
*)
val cycle_times : unit -> Time.Span.t Stream.t

(** [report_long_cycle_times ?cutoff ()] sets up something that will print
    a warning to stderr whenever there is an async cycle that is too long,
    as specified by [cutoff], whose default is 1s. *)
