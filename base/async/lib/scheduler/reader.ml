  ...
  let buf_len =
    ...
    | Some buf_len ->
        if buf_len > 0 then
          ...
          fail "Reader.create got non positive buf_len" (buf_len, fd)
          <:sexp_of< int * Fd.t >>
  in
  { fd = fd;
    id = Id.create ();
    ...
    `Wait
      (Deferred.create (fun result ->
        ...
        let ebadf () =
          ...
          fail "reader file descriptor was unexpectedly closed" t <:sexp_of< t >>
        in
      ...
      nonempty_buffer t (function
        | `Eof ->
          if ws_only then Ivar.fill result `Eof
            ...
            | Sexp.Cont _ -> fail "Reader.read_sexp got unexpected eof" t <:sexp_of< t >>
          end
        | `Ok ->
...
let read_bin_prot ?(max_len = 100 * 1024 * 1024) t bin_prot_reader =
  let error f =
    ksprintf (fun msg () -> fail "Reader.read_bin_prot" (msg, t) <:sexp_of< string * t >>)
      f
  in
  Deferred.create (fun result ->
    let handle_eof n =
...
let read_marshal_raw t =
  let eofn n =
    if n = 0 then `Eof
    else fail "Reader.read_marshal got EOF with bytes remaining" n <:sexp_of< int >>
  in
  ...
  Deferred.create (fun i ->
    ...
    >>> function
      ...
      | `Ok length_str ->
        ...
        | Error () ->
          fail "Reader.recv got strange length" (length_str, t) <:sexp_of< string * t >>
        | Ok length ->
...
let use t =
  let error s = fail "can not read from reader" (s, t) <:sexp_of< string * t >> in
  match t.state with
  | `Closed -> error "closed"
  | `In_use -> error "in use"
...
exception Load_sexp of string * [ `Error of exn | `Require_one_sexp_but_got of int ]
with sexp
...
let gen_load ?exclusive file
    (good : Sexp.t list -> ('a, exn) Result.t)
    (bad : Sexp.Annotated.t list -> ('a, exn) Result.t) =
  let load parse f =
    Monitor.try_with
      (fun () ->
    ...
    >>= function
      | Error error -> return (Error (Load_sexp (file, `Error error)))
      | Ok sexps -> f sexps
  in
...
let load_sexp_conv ?exclusive file f =
  let multiple sexps =
    Error (Load_sexp (file, `Require_one_sexp_but_got (List.length sexps)))
  in
...
let load_sexp_conv_exn ?exclusive file f =
  load_sexp_conv ?exclusive file f >>| Result.raise_error
;;

let load_sexp ?exclusive file = load_sexp_conv_exn ?exclusive file Fn.id

exception Load_sexps of string * exn list with sexp

let load_sexps_conv ?exclusive file f =
  gen_load ?exclusive file
    ...
    (fun annot_sexps ->
      let exns =
        List.filter_map annot_sexps ~f:(fun annot_sexp ->
          ...
          | Error exn -> Some exn)
      in
      Error (Load_sexps (file, exns)))
;;

let load_sexps_conv_exn ?exclusive file f =
  load_sexps_conv ?exclusive file f >>| Result.raise_error
;;

let load_sexps ?exclusive file = load_sexps_conv_exn ?exclusive file Fn.id
