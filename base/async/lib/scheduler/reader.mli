...
(** [transfer t pipe_w] transfers data from [t] into [pipe_w] one chunk at a time
    ...
    This funection will normally not be needed (see [pipe]). *)
...
(** [load_sexp file] reads a single sexp from [file].  It (asynchronously) fails
    if there is no such file, sexp, etc.  It also fails if there is something
    other than exactly one sexp in the file (of course sexp-style comments are
    allowed).
*)
val load_sexp : ?exclusive:bool -> string -> Sexp.t Deferred.t

(** [load_sexp_conv ?exclusive file ~f] load and convert the S-expression
    in a given [file] using [f], and return the deferred conversion result as
    a variant of either [Ok res] or [Error exn] otherwise.  This function is
    as efficient as [load_sexp] followed by conversion if there are no errors,
    but provides accurate error locations for failed type conversions. *)
val load_sexp_conv :
  ?exclusive:bool -> string -> (Sexp.t -> 'a) -> ('a, exn) Result.t Deferred.t

(** [load_sexp_conv_exn ?exclusive file ~f] like {!load_sexp_conv} it loads
    and converts the S-expression in a given [file] using [f], but returns
    the deferred result directly.  It raises an exception with accurate
    error locations if conversion fails. *)
val load_sexp_conv_exn :
  ?exclusive:bool -> string -> (Sexp.t -> 'a) -> 'a Deferred.t

(** [load_sexps file] load all sexps in a given file *)
val load_sexps : ?exclusive:bool -> string -> Sexp.t list Deferred.t

(** [load_sexps_conv file ~f] load and convert the S-expressions in a given
    [file] using [f], and return the deferred list of conversion results as
    variants of either [Ok res] or [Error exn] otherwise.  This function is as
    efficient as [load_sexps] followed by conversion if there are no errors,
    but provides accurate error locations for failed type conversions. *)
val load_sexps_conv :
  ?exclusive:bool
  -> string
  -> (Sexp.t -> 'a)
  -> ('a list, exn) Result.t Deferred.t

(** [load_sexps_conv_exn file ~f] like {!load_sexps_conv} it loads and
    converts the S-expressions in a given [file] using [f], but returns the
    list of results directly.  It raises an exception with accurate error
    locations if conversion fails. *)
