...
type 'a t = (File_descr.t, 'a Entry.t) Table.t Read_write.t with sexp_of
...
let create ~num_file_descrs =
  Read_write.create_fn (fun () ->
    Table.create ~num_keys:num_file_descrs ~key_to_int:File_descr.to_int)
;;

let request_start_watching t file_descr value read_or_write  =
  let table = table t read_or_write in
  if Table.mem table file_descr then
    Error (Error.arg "already watching file descr"
             <:sexp_of< File_descr.t * Read_write.Key.t >>
               (file_descr, read_or_write))
  else begin
    Table.replace table ~key:file_descr
      ~data:{ Entry.
              state = Watching;
              value;
            };
...
let request_stop_watching t file_descr read_or_write =
  ...
  | None ->
    Error (Error.arg "request stop watching got unknown file descr"
             <:sexp_of< File_descr.t * Read_write.Key.t >>
             (file_descr, read_or_write))
  | Some entry ->
...
let post_check (type a) t check_result =
  ...
  try
      ...
      `Ok (Read_write.mapi t ~f:(fun read_or_write table ->
        ...
        let ready =
          List.fold ready ~init:[] ~f:(fun ac file_descr ->
            ...
            | None ->
              fail "select returned unexpected ready file descr" file_descr
              <:sexp_of< File_descr.t >>
            | Some entry ->
              if Entry.stop_requested entry then
                ac
              else begin
      ...
      `Ok (Read_write.mapi t ~f:(fun read_or_write table ->
        ...
        let bad =
          List.fold (Read_write.get pre read_or_write) ~init:[] ~f:(fun ac file_descr ->
            match Table.find table file_descr with
            | None ->
              fail "Select returned unexpected bad file descr" file_descr
              <:sexp_of< File_descr.t >>
            | Some entry ->
              if Entry.stop_requested entry then
                ac
                ...
                | Error (Unix.Unix_error (Unix.EBADF, _, _)) ->
                  ...
                  entry.Entry.value :: ac
                | Error exn ->
                  fail "fstat raised unexpected exn" (file_descr, exn)
    ...
    | Error exn -> fail "select raised unexpected exn" exn <:sexp_of< exn >>
  with
  | exn ->
    fail "post_check bug" (exn, check_result, t)
    <:sexp_of< exn * a Check_result.t * a t >>
;;
