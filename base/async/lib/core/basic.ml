...
module Deferred = struct
  ...
  let debug_trace_connect = ref false

  let trace_connect s =
    if !debug_trace_connect (* deref, not logical negation *) then begin
      print_string "trace_connect: ";
      print_endline s;
    end;
  ;;
  ...
  let connect ivar tdef =
    ...
    if phys_equal t i
    then trace_connect "eq" (* Don't introduce cycles *)
    else begin
      ...
      | (Empty,   _)
        -> trace_connect "empty_star";
          t := Indir ivar
      | (Full v,  _)
        -> trace_connect "full_star";
          Ivar.fill ivar v
      | (Indir _, _)
        -> trace_connect "indir_star";
          assert false (* fulfilled by repr *)
      | (tc, Empty)
        -> trace_connect "star_empty";
          i := tc; (* do a swap *)
          t := Indir ivar
      ...
      | (Empty_many_handlers bag1, Empty_many_handlers bag2)
        -> trace_connect "many_many";
          Bag.transfer ~src:bag1 ~dst:bag2;
          debug_bag_check bag2;
          t := Indir ivar
      end;
      (* Squash original tdef reference; otherwise, it may grow by two indirections
         rather than one.  This is the key step that eliminates any references
         to [ivar] if it was an indirection. *)
      ignore (repr tdef)
    end;
  ;;
